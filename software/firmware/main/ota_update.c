...
static bool esp_ota_firm_parse_header(esp_ota_firm_t *ota_firm, const char *text,
                                      size_t total_len, ssize_t *parse_len)
{
	...
	*parse_len = 0;
	...
	while (text[i] != 0 && i < total_len) {
		...
		if (i_read_len > total_len - i) {
			ESP_LOGE(TAG, "Received malformed http header!");
		...
		if (i_read_len == 2) {
			if (ota_firm->content_len == 0) {
				ESP_LOGE(TAG, "Missing content-length parameter in header!");
...
static esp_err_t esp_ota_firm_parse_msg(esp_ota_firm_t *ota_firm, const char *in_buf,
                                        size_t in_len)
{
	size_t parse_bytes = 0;

	ESP_LOGD(TAG, "Input %zu bytes", in_len);
	...
	do {
		...
		ssize_t tmp;
		size_t parsed_bytes = in_len - parse_bytes;

		switch (ota_firm->state) {
			case ESP_OTA_INIT:
				if (esp_ota_firm_parse_header(ota_firm, in_buf + parse_bytes, in_len - parse_bytes, &tmp)) {
					if (tmp < 0) {
						ESP_LOGD(TAG, "Response is no OTA update");
				...
				ESP_LOGD(TAG, "continue");
				break;
			case ESP_OTA_PREPARE:
				ota_firm->read_bytes += in_len - parse_bytes;
				...
				break;
			case ESP_OTA_START:
				if (ota_firm->write_bytes + in_len - parse_bytes > ota_firm->ota_size) {
					ota_firm->bytes = ota_firm->ota_size - ota_firm->write_bytes;
					ota_firm->state = ESP_OTA_RECVED;
				} else {
					ota_firm->bytes = in_len - parse_bytes;
				}

				ota_firm->buf = in_buf + parse_bytes;
		...
		ESP_LOGD(TAG, "Parse %d bytes", parsed_bytes);

		parse_bytes += parsed_bytes;
...
bool ota_perform_update(void)
{
	...
	//ESP_LOGI(TAG, "Starting OTA example... @ %p flash %s", ota_perform_update,
	...
	if (configured != running) {
		ESP_LOGW(TAG, "Configured OTA boot partition at offset 0x%08x, but "
		              "running from offset 0x%08x",
	...
	if (!connect_to_http_server()) return false;
	...
	while (flag) {
		...
		if (buff_len < 0) { /*receive error*/
			...
			task_fatal_error();
		} else if (buff_len == 0 || esp_ota_firm_is_finished(&ota_firm)) {  /*packet over*/
			flag = false;
		...
		err = esp_ota_firm_parse_msg(&ota_firm, text, buff_len);
		if (err != ESP_OK) {
			return false;
		}
